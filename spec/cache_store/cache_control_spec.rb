...
describe Faraday::CacheStore::CacheControl do

  it 'takes a String with multiple name=value pairs' do
    instance = Faraday::CacheStore::CacheControl.new('max-age=600, max-stale=300, min-fresh=570')
  ...
  it 'takes a String with a single flag value' do
    instance = Faraday::CacheStore::CacheControl.new('no-cache')
  ...
  it 'takes a String with a bunch of all kinds of stuff' do
    instance =
      Faraday::CacheStore::CacheControl.new('max-age=600,must-revalidate,min-fresh=3000,foo=bar,baz')
    instance['max-age'].should == '600'
  ...
  it 'strips leading and trailing spaces' do
    instance = Faraday::CacheStore::CacheControl.new('   public,   max-age =   600  ')
  ...
  it 'ignores blank segments' do
    instance = Faraday::CacheStore::CacheControl.new('max-age=600,,max-stale=300')
    instance['max-age'].should == '600'
    instance['max-stale'].should == '300'
  end

  it 'sorts alphabetically with boolean directives before value directives' do
    instance = Faraday::CacheStore::CacheControl.new('foo=bar, z, x, y, bling=baz, zoom=zib, b, a')
  ...
  it 'responds to #max_age with nil when no max-age directive present' do
    instance = Faraday::CacheStore::CacheControl.new('public')
    instance.max_age.should be_nil
  end

  it 'responds to #shared_max_age with an integer when s-maxage directive present' do
    instance = Faraday::CacheStore::CacheControl.new('public, s-maxage=600')
  ...
  it 'responds to #public? non-truthfully when no public directive present' do
    instance = Faraday::CacheStore::CacheControl.new('private')
  ...
  it 'responds to #no_cache? non-truthfully when no no-cache directive present' do
    instance = Faraday::CacheStore::CacheControl.new('max-age=600')
    instance.should_not be_no_cache
  end

  it 'responds to #must_revalidate? truthfully when must-revalidate directive present' do
    instance = Faraday::CacheStore::CacheControl.new('must-revalidate')
  ...
  it 'responds to #proxy_revalidate? truthfully when proxy-revalidate directive present' do
    instance = Faraday::CacheStore::CacheControl.new('proxy-revalidate')
